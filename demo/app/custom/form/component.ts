...
function normalizeSchema(schema: any) {
  ...
  Object.keys(properties).forEach(key => {
    ...
    if (_schema.type === 'object' && !_schema.properties) {
      ...
      if (!_schema.items?.properties || Object.keys(_schema.items.properties).length === 0) {
        delete properties[key];
      }
    }
  });
  ...
  normalizeSchema(schema: any) {
    ...
    Object.keys(properties).forEach((key) => {
      ...
      if (_schema.type === 'object' && !_schema.properties) {
        ...
        this.normalizeSchema(_schema);
        if (Object.keys(_schema.properties).length === 0) {
          delete properties[key];
        }
      } else if (_schema.type === 'array' && _schema.items.type === 'object') {
        this.normalizeSchema(_schema.items);
